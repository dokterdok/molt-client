...
describe('Keychain', () => {
  ...
  describe('getGatewayToken', () => {
    it('should return the stored token', async () => {
      keychainStore['com.moltzer.client:gateway_token'] = 'my-secret-token';
    ...
    it('should return empty string on keychain error', async () => {
      ...
      vi.mocked(invoke).mockImplementationOnce(() => Promise.reject(new Error('Keychain access denied')));
  ...
  describe('setGatewayToken', () => {
    it('should store a token in the keychain', async () => {
      ...
      expect(keychainStore['com.moltzer.client:gateway_token']).toBe('new-token-123');
    });
    ...
    it('should overwrite an existing token', async () => {
      keychainStore['com.moltzer.client:gateway_token'] = 'old-token';
      
      await setGatewayToken('new-token');
      
      expect(keychainStore['com.moltzer.client:gateway_token']).toBe('new-token');
    });

    it('should delete the token when setting to empty string', async () => {
      keychainStore['com.moltzer.client:gateway_token'] = 'existing-token';

      await setGatewayToken('');

      // Should have called delete, not set with empty
      expect(keychainStore['com.moltzer.client:gateway_token']).toBeUndefined();
    });
    ...
    it('should handle special characters in token', async () => {
      ...
      expect(keychainStore['com.moltzer.client:gateway_token']).toBe(specialToken);
    });
    ...
    it('should handle very long tokens', async () => {
      ...
      expect(keychainStore['com.moltzer.client:gateway_token']).toBe(longToken);
    });
  });

  describe('deleteGatewayToken', () => {
    it('should delete the stored token', async () => {
      keychainStore['com.moltzer.client:gateway_token'] = 'to-delete';
    ...
    it('should handle keychain error on delete gracefully', async () => {
      vi.mocked(invoke).mockImplementationOnce(() => Promise.reject(new Error('Permission denied')));
