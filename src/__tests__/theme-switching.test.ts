...
const createMatchMediaMock = (defaultMatches = false) => {
  ...
  return vi.fn().mockImplementation((query: string) => ({
    ...
    addEventListener: vi.fn((type: string, listener: (ev: { matches: boolean }) => void) => {
...
describe('Theme Switching', () => {
  ...
  describe('theme transitions', () => {
    ...
    it('should not duplicate dark class on multiple dark theme applies', () => {
      ...
      const darkCount = Array.from(document.documentElement.classList).filter(c => c === 'dark').length;
